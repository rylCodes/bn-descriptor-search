...
async function initializeFaqs() {
  ...
  if (
    !removeFilter.classList.contains("hidden") &&
    !filterIndicator.classList.contains("hidden")
  ) {
...
function highlightActiveHeader() {
  ...
  const activeTH = Array.from(thElement).find(
    (th) => th.textContent === selectedOptionText
  );
  if (activeTH) {
    if (prevActiveTH) {
      prevActiveTH.classList.remove(
        "font-bold",
        "text-white",
        "cstm-txt-shadow"
      );
...
async function filterByKeyword(array, keyword, option) {
  const result = await array.filter((item) =>
    item[`${option}`].includes(keyword.toUpperCase())
  );
  return result;
}
...
async function displayResults(array, keyword, option) {
  ...
  const html = await filteredArr.map(
    (item) =>
      `<tr class=result-row>
